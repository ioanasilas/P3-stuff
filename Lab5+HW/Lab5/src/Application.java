...
public class Application
{
    ...
    int playGame(InputDevice h, int roundsToWin) throws IOException {
    ...
    void randomArraySort() throws IOException {
    ...
    int[] wordSizeHistogram(String sentence)
    {
        ...
        int[] wordFrequency = new int[30];

        for (String word : words)
        {
            int length = word.length();
            if (length < wordFrequency.length)
            {
                wordFrequency[length]++;
            }
            else
            {
                System.out.println("Encountered word over max len of 30: " + word);
            }
        }
    ...
    public void testFruitComparison(Collection <Fruit> fruits) throws IOException {
    ...
    // only have this work with inputDevice1
    void askUserForFile(InputDevice id)
    {
        while(true)
        {
            if (id.isTerminal()) {
                od.writeMessage("Enter file name please: ");
            }
            String fileName = id.getLine();
            try
            {
                // open file
                ...
                // print to terminal
                od.writeMessage(content.toString());
                // success so stop
                break;

            } catch (IOException e) {
                od.writeMessage("Error: File not found. Please enter another file name.");
            }
        }

    }

    void writeRandomNumbers(InputDevice id)
    {
        while(true)
        {
            if (id.isTerminal()) {
            ...
            try
            {
                ...
                od.writeMessage("Random numbers successfully written to " + fileName);
                break;

            } catch (IOException e) {
                od.writeMessage("Error: Unable to write to file. Please enter another file name.");
            }
